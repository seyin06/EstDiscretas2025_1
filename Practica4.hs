...
minimo (Raiz x izquierdo derecho) =
   if x <= minimo izquierdo && x <= minimo derecho
   then x
...
maximo (Raiz x izquierdo derecho) =
    if x >= maximo izquierdo && x >= maximo derecho
    then x
...
eliminar (Raiz x izquierda derecha) valor =
     if valor < x
...
recorrido (Raiz x izquierdo derecho) orden =
     if orden == InOrder
